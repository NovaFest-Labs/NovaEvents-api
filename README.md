...
| `GET` | `/api/events` | List all events (queries contract directly) |
...
## Background indexer (SQLite)

The API runs a background indexer that polls the on-chain contract and writes a JSON snapshot of each event into a local SQLite database.

- DB: a local SQLite file (default `./data/index.db`) is created automatically.
- Indexer: a background process polls the on-chain contract and writes a JSON payload per event into the `events_index` table.
- Default poll interval: every 30 seconds (configurable with `INDEX_SYNC_INTERVAL_MS`).
- Startup: the indexer is enabled by default; set `INDEXER_DISABLED=1` to disable it.

> **Note:** `GET /api/events` currently queries the contract directly (via parallel RPC calls) and does **not** read from the SQLite index. The index is written by the background indexer but is not yet wired into any read path. This means list responses always reflect live on-chain state at the cost of N+1 RPC calls per request.
